...
public:
	...
	DUCKDB_API virtual vector<OpenFileInfo> GlobFiltered(const string &path, FileOpener *opener, idx_t max_files);
	DUCKDB_API vector<OpenFileInfo> GlobFiles(const string &path, ClientContext &context,
	                                          FileGlobOptions options = FileGlobOptions::DISALLOW_EMPTY, idx_t max_files = 0);
