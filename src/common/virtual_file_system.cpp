...
vector<OpenFileInfo> VirtualFileSystem::GlobWithFilter(const string &path, const GlobFilterContext &filter_context, FileOpener *opener) {
	return FindFileSystem(path).GlobWithFilter(path, filter_context, opener);
}
